...
struct EditActionTests {
    ...
    @Test("kind-only verb maps every editable kind")
    func kindOnlyVerb() {
        #expect(editVerb(forKind: ClipboardRecord.kindText) == "edit")
        #expect(editVerb(forKind: ClipboardRecord.kindImage) == "crop")
        #expect(editVerb(forKind: ClipboardRecord.kindGif) == "crop")
        #expect(editVerb(forKind: ClipboardRecord.kindVideo) == "trim")
    ...
    @Test("video trims only when file exists")
    func videoGate() {
        let vid = makeRecord(kind: ClipboardRecord.kindVideo, plainText: nil)
        #expect(editActionVerb(for: vid, isBitmapImage: false, videoFileExists: true) == "trim")
