...
- **Verification:** Registration + approval persistence documented in `.claude/rules/smappservice-daemon.md`; Settings row reflects live status. Requires the user at the machine (the Developer ID cert is present locally — confirmed 2026-06-09).
...
## Review Amendments (2026-06-09)

A five-lens adversarial review (feasibility · security · scope · coherence · adversarial) returned **reconsider**, driven by the unrun U1 gate plus two security blockers. The human owns U1/U3-approval/U7 (physical lid/reboot/release); the buildable slice (U1 spike *script*, U2, U3 code, U4, U5, U6) proceeds autonomously with these binding amendments — they override the prose above where they conflict:

- **DECISION — no fallback (M6 resolved).** `PrivilegedCommand.swift` is deleted in U5 as the plan states; there is no sudo/osascript fallback. Macs that cannot approve the daemon (MDM-managed / standard accounts) get R3 guidance and Closed Lid stays unavailable until approved. Accepted to fully remove the legacy sudoers primitive.
- **B1 — fail-closed XPC, corrected mechanism.** "If `setCodeSigningRequirement` throws, the listener refuses every connection" is the wrong model for `NSXPCListener` — a swallowed error silently accepts every local caller. Instead: (1) validate the requirement string at daemon startup via `SecRequirementCreateWithString` and **`exit` the daemon** if it won't parse (a daemon must never run with no requirement); (2) on `macOS 14+` set `NSXPCConnection.setCodeSigningRequirement` per accepted connection inside `listener(_:shouldAcceptNewConnection:)`, returning `false` on any error/throw. Unit-test the decision logic: requirement-string-fails-to-parse → reject. (U7's different-identity test is still required but does NOT cover this bypass.)
- **B2 — drop the client `sudo -n <legacy-cleanup-script>` fallback entirely.** Its `lstat`-then-exec guards the file but not the parent dir → TOCTOU root hand-off. The daemon-side sweep is the only legacy-cleanup path. Removes a privilege path and a test scenario (updates R9, U5, the "Legacy cleanup" Key Decision, and the corresponding Risk).
- **M1 — fix the `SleepDisabled` parse (soft blocker — reconciliation correctness).** The inherited `output.contains("SleepDisabled") && output.contains("1")` false-positives on `standby 1` + `SleepDisabled 0`. Add a pure `parseSleepDisabled(fromPmsetG:)` in `DrobuShared` that reads the `SleepDisabled\s+([01])` value, unit-tested (incl. the `standby 1`/`SleepDisabled 0` → false case), used by **both** the daemon reconciliation table and `ClosedLidService`.
- **M3 — distinct daemon identifier.** Daemon `--identifier` = `com.danielius.ClipboardHistory.daemon` (≠ the app's `com.danielius.ClipboardHistory`), so the daemon does not satisfy its own client requirement. Add `codesign -d -r-` confirmation (daemon DR does NOT match the client string) to U2's verification.
- **M7 — terminate-time disable must not deadlock.** The bounded-wait disable on SIGTERM/`applicationWillTerminate` delivers its XPC reply on a **non-main** queue with a bounded timeout (~250–500ms); timeout is acceptable because the watchdog deadline is the guarantee. Never block `@MainActor` waiting on a reply routed to `@MainActor`.
- **M5 — corrected risk framing.** Net attack surface *changes shape* (transient/dormant/broad sudoers grant → permanent/live/narrow root XPC endpoint + state file + sweep); drop the "net surface plausibly lower" claim. The daemon is justified on its real grounds: Touch ID UX, the broken Authorization framework on 26.3, one approved component with a status API.
- **U2 build detail — SPM resources are target-scoped.** The daemon plist cannot be a `DrobuTests` resource read via `Bundle.module`. Keep it in `Sources/DrobuDaemon/` with an explicit `exclude:` (or `resources:`/copy as appropriate) and have the plist↔constants drift test read it via `#filePath`-relative path. State `DrobuDaemon`'s `exclude:` explicitly or the build hits "unhandled files."
- **M2 — pmset reboot-persistence note.** The shipped 2026-02-27 plan asserts `disablesleep` persists across reboots (man-page cited). The reconciliation table is built correct either way; the "SleepDisabled off post-reboot → re-apply" cell only fires when the state file shows a live session AND `SleepDisabled` reads off, so it is harmless if persistence holds. Confirm empirically in U7's reboot step.
- **Watch (fold into implementation):** `DaemonClient` reconnect backoff + max-retry; daemon log sanitizes XPC-derived values (numeric/enum only — no echoing strings) to prevent log injection; state-file on-read verification also `stat`s the parent dir; CI-verify Swift 6 / Sendable on the `@objc` reply blocks **early** (push WIP before the client cutover), not at the end.
...
- **Developer ID cert confirmed present (2026-06-09) — local daemon work is unblocked.** `security find-identity -v -p codesigning` on this machine lists the valid `Developer ID Application: DANIELIUS ISIŪNAS (TGL69S88MD)` identity (alongside the legacy self-signed `ClipboardHistoryDev`). So `build.sh` resolves the cert and the buildable units (U2/U4/U5/U6 + U3's `DaemonRegistrar` code) sign and run here today. (Supersedes the original "this laptop lacks the cert" risk, which was stale.) A *second* machine doing daemon work would still need the same identity imported.
